...
class CrowdSim(gym.Env):
    ...
    def step(self, action, update=True):
        ...
        if update:
            ...
            self.states.append([self.robot.get_full_state(), [human.get_full_state() for human in self.humans]])
            # if self.robot.policy == "ACTENVCARL":
        ...
        if mode == 'human':
            ...
            for k in range(len(self.states)):
                if k % 4 == 0 or k == len(self.states) - 1:
                    robot = plt.Circle(robot_positions[k], self.robot.radius, fill=True, color='k')
                    humans = [plt.Circle(human_positions[k][i], self.humans[i].radius, fill=True, color=cmap(i))
            ...
            ax.legend([robot, goal], ['Robot', 'Goal'], fontsize=fontsize)
            # ax.legend()
            ...
            # add time annotation
            time = ax.text(-1, 5, 'Time: {}'.format(0), fontsize=fontsize)
            ...
            if use_rate_statastic:
                ...
                ax2.set_xlabel('Number of EGRUs',fontsize=10)
                ...
                ax3.set_xlabel('Number of EGRUs', fontsize=10)
