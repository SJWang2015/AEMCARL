...
class MultiHumanRL(CADRL):
    ...
    def predict(self, state):
        ...
        if self.phase == 'train' and probability < self.epsilon:
            ...
            for action in self.action_space:
                ...
                if value > max_value:
                    # print("multi_human_rl step %d, %d, %d" % (self.model.step_cnt, self.model.step2_cnt, self.model.step3_cnt))
                    self.total_step += 1
                    # self.step_cnt += self.model.step_cnt
                    # self.step2_cnt += self.model.step2_cnt / 2
                    # self.step3_cnt += self.model.step3_cnt / 3
                    if self.use_rate_statastic:
                        self.step_cnt = self.model.step_cnt
                        self.step2_cnt = self.model.step2_cnt / 2
                        self.step3_cnt = self.model.step3_cnt / 3
                    
                    self.task_dist += np.linalg.norm((action.vx * self.time_step, action.vy * self.time_step))
                    max_value = value
                    max_action = action
    ...
    def compute_reward(self, nav, humans):
        ...
        elif (stationary_dist < self.env.stationary_dist) and (self.env.agent_prev_vx is not None and self.env.agent_prev_vy is not None) and stationary_state and not(reaching_goal):
            reward = self.env.stationary_penalty * (self.env.stationary_penalty_dist - stationary_dist) / self.env.stationary_penalty_dist
        else:
            reward = 0
