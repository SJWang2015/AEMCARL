...
class MultiHumanRL(CADRL):
    ...
    def predict(self, state):
        ...
        if self.phase == 'train' and probability < self.epsilon:
            ...
            #         max_action = ActionXY(0, 0)
            #     else: 
            #         max_action = ActionRot(0, 0)   
            print('max_value:'+ '\033[1;31;40m' + str(max_value) + '\033[0m')
    ...
    def compute_reward(self, nav, humans):
        ...
        for i, human in enumerate(humans):
            ...
            # print(nav.px, nav.py)
            if dist < 0.3 or abs(nav.px) >= 9.0 or abs(nav.py) >= 9.0:
                collision = True
                break
            if dist < dmin:
                dmin = dist
        ...
        reaching_goal = np.linalg.norm((nav.px - nav.gx, nav.py - nav.gy)) < 0.3
        ...
        if collision:
            ...
            reward = (dmin - 1.0) * 0.5 * self.time_step
            # info = Danger(dmin)
        elif (stationary_dist < 0.1) and stationary_state and not(reaching_goal):
            reward = -0.1 * (0.1 - stationary_dist) / 0.1
            info = Stationary()
        else:
            reward = 0
