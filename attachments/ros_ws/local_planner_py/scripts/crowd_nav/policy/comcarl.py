...
from crowd_nav.policy.cadrl import mlp, mlp2, bimlp
from crowd_nav.policy.multi_human_rl import MultiHumanRL

class ValueNetwork(nn.Module):
    def __init__(self, gpu, input_dim, self_state_dim, joint_state_dim, in_mlp_dims, sort_mlp_dims, action_dims, sort_attention_dims, nheads, dropout, with_global_state):
        ...
        #     self.sort_mlp = bimlp(self.in_mlp_dims[-1]*2+self.joint_state_dim, sort_mlp_dims)     
        # else:
        #     self.sort_mlp = bimlp(self.in_mlp_dims[-1]*2, sort_mlp_dims)
        # avg+mlp1 For CARLA
        if self.with_global_state:
...
class COMCARL(MultiHumanRL):
    """
    Simple CommNet layer, similar to https://arxiv.org/pdf/1605.07736.pdf
    """
    def __init__(self):
        super(COMCARL, self).__init__()
        self.name = 'COMCARL'

    def configure(self, config):
        self.set_common_parameters(config)
        global_state_dim = config.getint('comcarl', 'global_state_dim')
        dropout = config.getfloat('comcarl','dropout')
        alpha = config.getfloat('comcarl','alpha')
        nheads = config.getint('comcarl','nheads')
        in_mlp_dims = [int(x) for x in config.get('comcarl', 'in_mlp_dims').split(', ')]
        sort_mlp_dims = [int(x) for x in config.get('comcarl', 'sort_mlp_dims').split(', ')]
        sort_attention_dims = [int(x) for x in config.get('comcarl', 'sort_attention_dims').split(', ')]
        action_dims = [int(x) for x in config.get('comcarl', 'action_dims').split(', ')]
        # attention_dims = [int(x) for x in config.get('comcarl', 'attention_dims').split(', ')]
        self.with_om = config.getboolean('comcarl', 'with_om')
        with_global_state = config.getboolean('comcarl', 'with_global_state')
        self.multiagent_training = config.getboolean('gatcarl', 'multiagent_training')
        self.model = ValueNetwork(False, self.input_dim(), self.self_state_dim, self.joint_state_dim, in_mlp_dims, sort_mlp_dims, action_dims, 
